...
export class Interface extends events.EventEmitter {
  ...
  async start() {
    ...
    try {
      ...
      const stdinConfig = await this._getEnvConfig() as any;
  ...
  async _getEnvConfig() {
    return new Promise(function(resolve, reject) {
      ...
      if (process.env.PG3INIT) {
        ...
        resolve(JSON.parse(data.toString('ascii')) as any);
      }
    });
  }
    ...
    this._mqttClient.on('message', (topic, message) => {
      ...
      if (message.length) {
        try {
          ...
          errorStack(err, 'Error processing %s:',
  ...
  _onResult(messageKey, messageContent) {
    ...
    if (trackedCommands.includes(messageKey)) {
      ...
      const trackedRequest =
        _this._messageAsyncTracking[messageKey + '-' + address];
      if (trackedRequest) {
  ...
  async addNode(node: any) {
    if (!(node instanceof Node)) {
      ...
      return await this.sendMessageAsync(
        'addnode-' + node.address, message, 'command');
    }
  }
  ...
  setCustomParamsDoc(html) {
    if (typeof html !== 'string') {
      ...
      this.sendMessage(
        {set: [{key: 'customparamsdoc', value: html}]}, 'custom');
    }
  }
