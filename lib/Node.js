...
export const nodeDefId = 'UNDEFINED';
...
export class Node {
    ...
    convertValue(driver, value) {
        ...
        switch (typeof value) {
            ...
            case 'boolean':
                ...
                if (uom !== 2) {
                    logger.warn('Value for driver %s is a boolean, but the uom is %s (Should be 2)', driver, uom);
                }
                return value ? '1' : '0';
            default:
                return value;
    ...
    setDriver(driver, value, report = true, forceReport = false, uom = null) {
        ...
        if (driver in this.drivers &&
            ...
            if (this.drivers[driver].value !== value) {
                logger.info('Setting node %s driver %s: %s', this.address, driver, value);
    ...
    reportCmd(command, value = null, uom = null) {
        const message = {
            command: [{
                    address: this.address,
                    cmd: command
    ...
    async runCmd(cmdMessage) {
        ...
        if (!nodeCommandFunction) {
            logger.error('Node %s using nodeDefId %s does not have a command: %s', this.address, this.id, cmdMessage.cmd);
        }
