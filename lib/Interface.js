...
module.exports = class Interface extends events.EventEmitter {
    ...
    Object.keys(message).forEach(function(messageKey) {
      ...
      switch (messageKey) {
        ...
        case 'setLogLevelList':
          logger.info('%s: response message %o', messageKey, messageContent);
          break;
  ...
  async _onMessageQueued(opt) {
    ...
    if (!this._shuttingDown) {
      ...
      switch (messageKey) {
        ...
        case 'customdata':
          this.emit('customData', messageContent);
          break;

        case 'customtypeddata':
          this.emit('customTypedData', JSON.parse(messageContent));
          break;

        case 'customparams':
          this.emit('customParams', JSON.parse(messageContent));
          break;

        case 'nsdata':
          this.emit('nsData', messageContent);
          break;

        case 'customtypedparams':
          this.emit('customTypedParams', JSON.parse(messageContent));
          break;
        ...
        case 'getAll':
          ...
          messageContent.forEach(function(msg) {
            logger.error('getAll sub message: %o', msg);
            _this._queue.add({
              // We set it here to facilitate routing to the proper function
              messageKey: msg['key'],
        ...
        case 'addnode':
          ...
          if (messageContent.hasOwnProperty('address')) {
            // TODO: in Python, we kick off a thread to run the start function
            this.emit('addNodeDone', messageContent);
          }
          break;
  ...
  _onConfig(config) {
    ...
    config.nodes.forEach(function(n) {
      ...
      if (!_this._nodes[address]) {
        ...
        //const primary = n.primaryNode.slice(5);
        const primary = n.primaryNode;

        logger.error('address = %s, primary = %s', address, primary);
  ...
  _sendMessage(message, type, retain = false) {
    ...
    logger.error(`_sendMessage: sending ${JSON.stringify(message)} to ${topic}`);
