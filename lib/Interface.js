...
module.exports = class Interface extends events.EventEmitter {
  ...
  constructor(declaredNodeClasses) {
    ...
    this._noticesFormatObject = false;
  ...
  addNotice(key, text) {
    if (this._noticesFormatObject) {
      const message = { set: [{key:'notices', value:{ key: key, value: text }}]};
  ...
  removeNoticesAll() {
    ...
    if (this._noticesFormatObject) {
      Object.keys(this.getNotices()).forEach(function(key) {
        const message = { removenotice: { key: key } };
        _this.sendMessage(message, 'custom');
      });
    } else {
      this.addCustomData({keyedNotices: {}});

      this.getNotices().forEach(function(text) {
        // We can delete by using the text.
        const message = { removenotice: text };
        _this.sendMessage(message, 'custom');
      });
    }
  }
