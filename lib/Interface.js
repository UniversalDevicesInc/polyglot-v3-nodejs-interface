...
module.exports = class Interface extends events.EventEmitter {
  ...
  _mqttStart() {
    ...
    const re = /:/g;
    const clientId = (this._prefix || this._uuid) + '_' + this._profileNum;

    const username = (this._prefix || this._uuid)
    .replace(re, '') + '_' + this._profileNum;
    ...
    if (fs.existsSync(username + '.cert')) {
      ...
      options = {
        ...
        will: {
          topic: 'udi/pg3/connections/ns/' + this.id,
    ...
    this._mqttClient.on('connect', () => {
      try {
        ...
        if (use_mosquitto)
          _this._mqttClient.publish('udi/pg3/connections/ns/' + this.id,
            '{"connected": [{}]}',
            true);
