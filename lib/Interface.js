...
export class Interface extends events.EventEmitter {
    ...
    _mqttPort;
    _mqttTopic;
    _mqttTopicPolyglotConnection;
    _profileNum;
    _mqttClient;
    _mqttClientConnected;
    _mqttPolyglotConnected;
    _config;
    _queue;
    ...
    constructor(declaredNodeClasses) {
        ...
        this._queue = new Queue(this._onMessageQueued, this, 'Message Queue Processor');
    ...
    async start() {
        ...
        try {
            ...
            errorStack(err, 'Could not get and process config:');
        }
    }
    ...
    async ready() {
        ...
        this._sendMessage({ getIsyInfo: {} }, 'system');
    ...
    _mqttStart() {
        const _this = this;
        const re = /:/g;
        const username = this.id.replace(re, '');
        var use_mosquitto = false;
        var options = {};
        if (fs.existsSync(username + ".cert")) {
            var KEY = fs.readFileSync(username + ".key");
            var CERT = fs.readFileSync(username + ".cert");
            options = {
                port: this._mqttPort,
                clientId: this.id,
                username: username,
        ...
        this._mqttClient.on('connect', () => {
            try {
                ...
                if (use_mosquitto)
                    _this._mqttClient.publish('udi/pg3/connections/ns/' + this.id, '{"connected": [{}]}', { retain: true, qos: 1 });
                _this._mqttClient.subscribe(_this._mqttTopic);
                _this.emit('mqttConnected');
                _this.ready();
            }
            catch (err) {
                errorStack(err, 'Error on MQTT connect handler:');
            }
        });
            ...
            if (message.length) {
                try {
                    ...
                    if (topic === _this._mqttTopicPolyglotConnection) {
                        /* TODO: We don't get this anymore,
                            remove mqttPolyglotConnected flag */
                    ...
                    errorStack(err, 'Error processing %s:', _this._mqttTopicPolyglotConnection);
                }
            }
        });
        ...
        this._mqttClient.on('close', () => {
            if (use_mosquitto)
                _this._mqttClient.publish('udi/pg3/connections/ns/' + this.id, '{"disconnected": [{}]}', { retain: true, qos: 1 });
            _this._mqttClientConnected = false;
            ...
            switch (messageKey) {
                ...
                default:
                    if (queuedMessages.includes(messageKey)) {
                        ...
                        logger.error('Invalid message %s received %o:', messageKey, message);
                    }
            }
        });
    }
        ...
        if (!this._shuttingDown) {
            ...
            switch (messageKey) {
                ...
                case 'customparamsdoc':
                    ...
                    break;
                case 'setLogLevel':
                    if (messageContent.level !== '') {
                        this.setLogLevel(messageContent.level);
                    }
                    break;
                case 'oauth':
                    if (messageContent !== '') {
                        this.emit('oauth', JSON.parse(messageContent));
                    }
                    break;
                case 'webhook':
                    if (messageContent !== '') {
                ...
                case 'removenode':
                    ...
                    break;
                default:
                    logger.error('Invalid queued message %s received %o:', messageKey, messageContent);
            }
        }
        else {
    ...
    _addNodeToList(node) {
        ...
        if (!this._nodes[node.address]) {
            ...
            if (NodeClass) {
                newNode = new NodeClass(node.nodeDefId, this, primary, node.address, node.name);
                this._nodes[node.address] = newNode;
            }
            else {
                logger.error('Config node with address %s has an invalid class %s', node.address, node.nodedef);
            }
        }
        else {
    ...
    _onResult(messageKey, messageContent) {
        ...
        if (trackedCommands.includes(messageKey)) {
            ...
            try {
                ...
                errorStack(err, 'Error on Received result:');
            ...
            logger.info('Received result for unhandled command %s: %o', messageKey, messageContent);
        }
    }
    ...
    _sendMessage(message, type, retain = false) {
        ...
        logger.debug(`_sendMessage: sending
      ${JSON.stringify(message)} to ${topic}`);
        _this._mqttClient.publish(topic, JSON.stringify(message), { retain: retain, qos: 1 });
    }
    ...
    isConnected() {
        ...
        if (!connected) {
            logger.warn('Polyglot connection is not connected. ' +
                'MQTT Client is%s connected, Polyglot is%s connected', this._mqttClientConnected ? '' : ' NOT');
        }
        return connected;
    }
        ...
        if (!(node instanceof Node)) {
            ...
            const message = {
                addnode: [{
                        ...
                        drivers: Object.keys(node.drivers)
                            .map(function (key) {
                            return Object.assign({}, node.drivers[key], { driver: key });
                        }),
                        hint: null
                    }],
            };
    ...
    getNodesFromDb(address = null) {
        ...
        if (address === null) {
            ...
            return this._nodesFromDb.find((node) => node.address === address);
        }
        else if (Array.isArray(address)) {
            return this._nodesFromDb.filter((node) => address.includes(node.address));
        }
        return null;
    }
    ...
    *nodes() {
    ...
    setPoll(shortPoll = null, longPoll = null) {
        const message = { setpoll: {} };
        if (shortPoll !== null) {
    ...
    webhookResponse(response, status = 200) {
        const message = {
            webhook: {
                response: response,
                status: status
    ...
    bonjour(type, protocol = '_tcp', subtypes = []) {
    ...
    // Update JSON profile
    async updateJsonProfile(profile, options = {}) {
        const waitResponse = options.waitResponse || false;
        const message = {
