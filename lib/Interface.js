...
module.exports = class Interface extends events.EventEmitter {
  ...
  _mqttStart() {
    ...
    const certName = this._prefix ?
      this._prefix.replace(re, '') + '_' + this._profileNum :
      username;
    var use_mosquitto = false;
    var options = {};

    if (this._secure && fs.existsSync(certName + '.cert')) {
      var KEY = fs.readFileSync(certName + '.key');
      var CERT = fs.readFileSync(certName + '.cert');
    ...
    this._mqttClient.on('connect', () => {
      try {
        ...
        if (use_mosquitto)
          _this._mqttClient.publish('udi/pg3/connections/ns/' + this.id, '{"connected": [{}]}', true);
    ...
    this._mqttClient.on('close', () => {
      if (use_mosquitto)
        _this._mqttClient.publish('udi/pg3/connections/ns/' + this.id, '{"disconneted": [{}]}', true);
      _this._mqttClientConnected = false;
